...
if(filter_has_var(INPUT_GET, "getStudentTypeId")) {
    $getStudentTypeId = $_REQUEST['getStudentTypeId'];
    $GetPC = $common->GetRows("SELECT * FROM tbl_student_types WHERE id = '{$getStudentTypeId}'"); 
    foreach ($GetPC as $gsdata) 
    {
        $getStudentType = $gsdata['StudentType'];
        $getisActive = $gsdata['isActive'];
    }
}

// Update School Details 
if(filter_has_var(INPUT_POST, "Update_StudentType")) {
    try  
        {   
            $EditStudentType =$common->CCStrip($_POST['EditStudentType']);
            $Update_StudentType =$common->CCStrip($_POST['Update_StudentType']);
           
            $common->Insert("UPDATE tbl_student_types SET StudentType = '{$EditStudentType}', isActive = '{$AcademicYearStatus}' WHERE id = '{$Update_StudentType}' ");
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="form-group">
                <label>Edit Student Type</label>
                <input type="text" class="form-control" name="EditStudentType" id="EditStudentType" placeholder="School /Institution Name"  autocomplete="off" required value="<?php echo $getStudentType; ?>">
                <input type="hidden" class="form-control" name="Update_StudentType" id="Update_StudentType" value="<?php echo $getStudentTypeId; ?>">
            </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
...
<center class=" r_corners m_top_20">
    <h4 class="m_top_20 m_bottom_20">Please wait... Updating Student Type Details</h4>
...
<div class="alert alert-success alert-dismissible">
    ...
    <h4><i class="icon fa fa-database"></i> Student Type Successfully Updated!</h4>  
</div>
</div>
    ...
    e.preventDefault(); 
        if($('#UpdateDepartmentFRM').valid()) { 
          ...
          var formData = new FormData($(this)[0]); 
            $.ajax({
                url: 'ajax-edit-student-type.php',
                type: 'POST',
                data: formData,
                async: true,
                success: function () {
