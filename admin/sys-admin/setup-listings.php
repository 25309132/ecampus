...
$common=new common();
try {
// 1. Insurance Schemes List
if($_GET["action"] == "GetInsuranceSchemesList") {
	if(isset($_REQUEST["SearchIFSNameP"]) || isset($_REQUEST["SearchInsuranceFirmID"])) {
		$SearchIFSNameP = $_REQUEST["SearchIFSNameP"];
		$SearchInsuranceFirmID = $_REQUEST["SearchInsuranceFirmID"];

		if(!empty($SearchIFSNameP) && !empty($SearchInsuranceFirmID)) {
			$where = "WHERE (su.insuranceSchemeName LIKE '%".$SearchIFSNameP."%' OR su.insuranceSchemeDescription LIKE '%".$SearchIFSNameP."%') AND  su.insuranceFirmID = '{$SearchInsuranceFirmID}'  ";
		}
		else {
			$where = " WHERE su.insuranceFirmID = '{$SearchInsuranceFirmID}'  ";
		}
	}
	else {
		$where = " ";
	...
	//Get record count
	$recordCount=$common->CCGetDBValue("SELECT COUNT(*) AS RecordCount FROM tbl_insurance_schemes `su` ".$where." ;");
	//Get records from database
	$result = $common->GetRows("SELECT su.*, FORMAT(su.schemeTotalCoverAmount, 2) AS schemeTotalCoverAmount, FORMAT(su.schemeDrugsCoverAmount, 2) AS schemeDrugsCoverAmount, FORMAT(su.schemeProceduresCoverAmount, 2) AS schemeProceduresCoverAmount, gif.insuranceName FROM tbl_insurance_schemes `su` LEFT JOIN tbl_insurance_firms gif ON gif.id = su.insuranceFirmID ".$where." ORDER BY " . $_GET["jtSorting"] . " LIMIT " . $_GET["jtStartIndex"] . "," . $_GET["jtPageSize"] . ";");
...
// Get Insurance Scheme Types
elseif($_GET["action"] == "GetSchemeTypeID")
	{
		$result = $common->GetRows("SELECT schemeTypeName AS DisplayText, id AS Value FROM `setup_scheme_types`");
		$jTableResult = array();
		$jTableResult['Result'] = "OK";
